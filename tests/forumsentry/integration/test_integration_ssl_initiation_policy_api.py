...
class TestIntegrationSslInitiationPolicyApi(TestIntegration):


    def test_integration_ssl_initiation_policy_api(self):
        '''
            Integration tests for SslInitiationPolicyApi
        '''
        ...
        model = self.loadModel(test_name, SslInitiationPolicy)
        name = test_name + self._unique_id
        model.name = name
        #model.key_pair = key_pair_name
        
        #verify we loaded the right model
        self.assertIsInstance(model, SslInitiationPolicy)
        self.assertEqual(model.name, name)
        
        #create the api to test
        api = ssl_initiation_policy_api.SslInitiationPolicyApi(self._conf)
        ...
        self.assertIsInstance(retrieved, SslInitiationPolicy)
