...
class Config(object):
    ...
    def __init__(self, host=None, port=None, protocol=None, context=None, username=None, password=None, verify_ssl=True, ssl_ca_file=None):
        ...
        if os.getenv("FSSENTRY_DEBUG", None) is not None:
            self._debug = True
            self._enable_debug()
    ...
    def forumsentry_url(self):
        ...
        self._logger['package_logger'].debug("url: {0}".format(url))
        return url
    ...
    def basic_authentication_header(self):
        ...
        if not self._username:
            self._logger['package_logger'].warn("username not set")
        if not self._password:
            self._logger['package_logger'].warn("password not set")
        ba_auth_header = urllib3.util.make_headers(basic_auth=self._username + ':' + self._password).get('authorization')
        
        self._logger['package_logger'].debug("ba_auth_header: {0}".format(ba_auth_header))
